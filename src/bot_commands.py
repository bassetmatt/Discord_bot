...
class Music(commands.Cog):
    ...
    @commands.command(name='ping', aliases=['Ping', 'ping!', 'Ping!'])
    async def _help(self, ctx: commands.Context):
        """
    ...
    @commands.command(name='kira', aliases=['yoshikage', 'mynameis'])
    async def _kira(self, ctx: commands.Context):
        """
        Writes the Yoshikage Kira copypasta.
        """
        return await ctx.send("My name is Yoshikage Kira. I'm 33 years old. "\
    ...
    @commands.command(name='darthplagueis', aliases=['DarthPlagueis', 'plagueis', 'Plagueis'])
    async def _darthplagueis(self, ctx: commands.Context):
        """
        Writes the Darth Plagueis copypasta.
        """
        return await ctx.send("Did you ever hear the tragedy of Darth Plagueis the wise ? "\
            "I thought not. It's not a story the Jedi would tell you. It's a Sith legend. "\
            "Darth Plagueis was a Dark Lord of the Sith, so powerful and so wise he could use the Force to "\
            "influence the midichlorians to create life... He had such a knowledge of the dark side that he "\
            "could even keep the ones he cared about from dying. The dark side of the Force is a pathway to many "\
            "abilities some consider to be unnatural. He became so powerful... the only thing he was afraid of "\
    ...
    @commands.command(name='hellothere', aliases=['HelloThere',"hellothere!", "HelloThere!"])
    async def _hellothere(self, ctx: commands.Context):
        """
        Answers "General Kenobi!" to "Hello there!".
        """
        return await ctx.send("General Kenobi!")
